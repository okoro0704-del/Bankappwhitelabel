-- Store account-holder temporary password for admin deliverables.
-- Default password to username when none is provided.

alter table public.profiles
  add column if not exists handoff_temp_password text;

comment on column public.profiles.handoff_temp_password is
  'Temporary password shown in admin deliverables for the account holder. Clear after rotation.';
...
begin
  ...
  -- Default temporary password to username when none provided.
...
-- Allow tenant admins to clear the stored temp password after handoff.
create or replace function public.admin_clear_user_temp_password(p_profile_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  set handoff_temp_password = null, updated_at = now()
  where id = target.id;
end;
$$;

revoke all on function public.admin_clear_user_temp_password(uuid) from public;
grant execute on function public.admin_clear_user_temp_password(uuid) to authenticated;
