-- Default transfer PIN for account holders is always 1111 (shown in admin deliverables).
...
-- Align existing customer accounts to the default deliverable PIN.
update public.profiles p
set
  transfer_pin_hash = extensions.crypt('1111', extensions.gen_salt('bf', 10)),
  handoff_transfer_pin = '1111',
  updated_at = timezone('utc', now())
where p.role = 'user';

-- Also set PIN on profile create so deliverables RPC returns 1111 immediately
-- (account insert trigger only runs after profile row is snapshotted in create RPC).
...
comment on column public.profiles.handoff_transfer_pin is
  'Temporary transfer PIN shown in admin deliverables. Default is 1111 until the customer changes it.';
